# Discway - Платформа Аренды Инструментов и Оборудования
...
> **Современная платформа для аренды инструментов и оборудования между пользователями, построенная на Node.js и Express**
...
**Discway** - это комплексная платформа для аренды инструментов и оборудования, которая позволяет пользователям арендовать различные предметы у других пользователей, управлять собственными вещами для аренды и создавать сообщество вокруг совместного использования ресурсов. Платформа облегчает совместное использование вещей между пользователями с такими функциями, как аутентификация пользователей, управление продуктами, планирование аренды, отзывы и избранное.

### Что Делает Этот Проект

- **Управление Пользователями**: Регистрация, аутентификация и управление профилем с JWT токенами
- **Каталог Продуктов**: Пользователи могут создавать, управлять и просматривать доступные для аренды предметы
- **Система Аренды**: Бронирование предметов на определенные даты с рабочим процессом одобрения
- **Система Отзывов**: Оценка и отзывы о предметах после завершения аренды
- **Избранное**: Сохранение предметов в личный список избранного
- **Поиск и Фильтрация**: Поиск предметов по категории, ценовому диапазону, рейтингам и многому другому

### Ключевые Возможности

- **Безопасная Аутентификация**: JWT-аутентификация пользователей с хешированием паролей bcrypt
- **Рабочий Процесс Аренды**: Управление статусом аренды: Ожидание → Принято/Отклонено
- **Система Рейтингов**: Автоматический расчет рейтинга продукта на основе отзывов пользователей
- **Предотвращение Конфликтов Дат**: Предотвращает двойное бронирование предметов
- **Роли Пользователей**: Различие между владельцами предметов и арендаторами
- **Комплексное API**: RESTful эндпоинты для всей функциональности платформы
...
| **Фреймворк** | Express.js | 4.21+ |
| **База Данных** | PostgreSQL + Sequelize ORM | Последняя |
| **Аутентификация** | JWT (JSON Web Tokens) | 9.0+ |
| **Безопасность Паролей** | bcrypt | 5.1+ |
| **CORS** | Cross-Origin Resource Sharing | Включено |
| **Окружение** | dotenv | 16.4+ |

### База Данных
- **СУБД**: PostgreSQL
- **ORM**: Sequelize.js
- **Модели**: User, Product, Rent, Review, Favourite
- **Связи**: Правильные ограничения внешних ключей и ассоциации

### Архитектура
- **MVC Паттерн**: Разделение контроллеров, моделей и маршрутов
- **Middleware**: Middleware для аутентификации и обработки ошибок
- **Дизайн API**: RESTful API эндпоинты
- **Обработка Ошибок**: Централизованная обработка ошибок с пользовательскими классами ошибок
...
│   ├── productController.js  # Операции с продуктами
│   ├── rentController.js     # Управление арендой
│   ├── reviewController.js   # Система отзывов
│   └── favouriteController.js # Управление избранным
├── models/              # Модели базы данных и ассоциации
│   └── models.js
├── routes/              # Определения API эндпоинтов
│   ├── userRouter.js
│   ├── productRouter.js
│   ├── rentRouter.js
│   ├── reviewRouter.js
...
├── error/               # Пользовательские классы ошибок
...
Платформа использует реляционную базу данных с пятью основными сущностями:

| Сущность | Описание | Ключевые Поля |
|--------|-------------|------------|
| **Пользователи** | Пользователи платформы с данными аутентификации и профиля | `idUser`, `name`, `email`, `phone`, `password` |
| **Продукты** | Предметы, доступные для аренды с описаниями и ценами | `idProduct`, `userId`, `name`, `description`, `price`, `rating` |
| **Аренда** | Транзакции аренды с датами и статусом | `idRent`, `idUser`, `idProduct`, `status`, `dataStart`, `dataEnd` |
| **Отзывы** | Обратная связь и оценки пользователей для продуктов | `idReview`, `idUser`, `idProduct`, `rate`, `comment` |
| **Избранное** | Предметы, сохраненные пользователями для быстрого доступа | `idFavourite`, `userId`, `idProduct` |

---

## API Эндпоинты

Платформа предоставляет комплексные REST API эндпоинты для:

- **Управления Пользователями**: Регистрация, вход и операции с профилем
- **Операций с Продуктами**: CRUD операции для предметов аренды
- **Управления Арендой**: Создание, просмотр и управление запросами на аренду
- **Системы Отзывов**: Создание, чтение, обновление и удаление отзывов
- **Избранного**: Добавление, удаление и просмотр избранных предметов
...
```bash
npm run dev          # Запустить сервер разработки с nodemon
npm start            # Запустить продакшн сервер
npm test             # Запустить тесты (если настроены)
npm run build        # Собрать для продакшена (если настроено)
```
